...
RSpec.describe Job, type: :model do
  ...
  context "#job_duration" do
    it "shows the duration between that latest job start and finish" do
      j = create(:job)
      q = create(:status, job: j, name: "started", created_at: "2023-04-12 01:00:00Z")
      q = create(:status, job: j, name: "started", created_at: "2023-04-12 01:10:00Z")
      q = create(:status, job: j, name: "complete", created_at: "2023-04-12 01:20:00Z")
      expect(j.job_duration).to eq(10*60)
    end
    it "returns nil if the job is only queued" do
      j = create(:job)
      q = create(:status, job: j, name: "queued")
      expect(j.job_duration).to be_nil
    end
    it "returns nil if the job is only queued and started" do
      j = create(:job)
      q = create(:status, job: j, name: "queued")
      q = create(:status, job: j, name: "started")
      expect(j.job_duration).to be_nil
    end
  end
  context "#queued_to_completion_duration" do
    it "shows the duration from queue time to finish" do
      j = create(:job)
      q = create(:status, job: j, name: "queued", created_at: "2023-04-12 01:00:00Z")
      q = create(:status, job: j, name: "started", created_at: "2023-04-12 01:05:00Z")
      q = create(:status, job: j, name: "started", created_at: "2023-04-12 01:10:00Z")
      q = create(:status, job: j, name: "complete", created_at: "2023-04-12 01:20:00Z")
